...
invert = T.GriffinLim(
    n_fft=2048, n_iter=32, hop_length=1, win_length=2048, power=2
)
...
axs[2].imshow(
    librosa.power_to_db(spec_masked.numpy()), aspect="auto", origin="lower"
)
axs[3].plot(time, eod_inv.numpy())
axs[4].imshow(
    librosa.power_to_db(inv_spec.numpy()), aspect="auto", origin="lower"
)
plt.show()
